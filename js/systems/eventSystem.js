...
function queueDelayedEffects(state, delayed = []) {
  delayed.forEach((item) => {
    ...
    state.pendingEffects.push({ inMin: item.inMin, effects: item.effects });
  });
}
...
function chooseCandidate(candidates = []) {
  ...
  const weighted = candidates.map((event) => ({
    event,
    chance: event.chancePerHour * (EVENT_ROLL_INTERVAL_MIN / 60)
  }));
  const ready = weighted.filter((item) => rand() < item.chance).map((item) => item.event);
  if (!ready.length) return null;
  return ready[Math.floor(rand() * ready.length)];
}
...
export function maybeTriggerEvent(state, events) {
  ...
  const candidates = events.filter((event) => evalCondition(state, event.condition));
  ...
  state.nextEventAt = Date.now() + EVENT_ROLL_INTERVAL_MIN * 60 * 1000;

...
import { maybeTriggerEvent, tickActiveEvent, tickDelayedQueue } from '../systems/eventSystem.js';
...
export function updateEngine(state, events) {
  ...
  if (state.currentEvent) pauseEngine(state, 'event-sheet');
  else resumeEngine(state, 'event-sheet');
